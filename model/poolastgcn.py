...
class ASTGCNBlock(nn.Module):
    ...
    def forward(
        ...
        edge_attr: Union[torch.LongTensor, List[torch.LongTensor]],
    ) -> torch.FloatTensor:
        """
        ...
        if not isinstance(edge_index, list):
            data = Data(
                edge_index=edge_index, edge_attr=edge_attr, num_nodes=num_of_vertices
            )
            ...
            for t in range(num_of_timesteps):
                data = Data(
                    edge_index=edge_index[t].detach(), edge_attr=edge_attr[t].detach(), num_nodes=num_of_vertices
                )
                if self._normalization != "sym":
                    lambda_max = LaplacianLambdaMax()(data).lambda_max
                else:
...
class PoolASTGCN(nn.Module):
    ...
    def __init__(
        ...
        super(PoolASTGCN, self).__init__()
        self.tempool = tempool
        ...
        # add tempool
        if len(tempool) > 0:
            v_num_list = [int(num_of_vertices*ratio) for ratio in tempool[0]]
            t_num_list = [int(len_input*ratio) for ratio in tempool[1]]
            self.st_pooling = TemPool(device, nb_time_filter, v_num_list, t_num_list)
            self._blocklist.extend(
                [
                    ASTGCNBlock(
                        ...
                        1,
                        v_num_list[i],
                        t_num_list[i] // time_strides,
                        normalization,
                        bias,
                    )
                    for i in range(len(tempool[0]))
                ]
            )
            self._final_conv = nn.Conv2d(
                int(t_num_list[-1] / time_strides),
                num_for_predict*out_dim,
                kernel_size=(v_num_list[-1], nb_time_filter),
            )
            ...
            self._final_conv = nn.Conv2d(
                ...
                num_for_predict*out_dim,
                kernel_size=(num_of_vertices, nb_time_filter),
            )
    ...
    def forward(
        ...
        X = self._final_conv(X.permute(0, 3, 1, 2)) # (B, T*C, 1, 1)
